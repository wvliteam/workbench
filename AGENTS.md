...
- **`integrator`（条件集成角色）**：本地环境部署与集成联调专家。当方案涉及跨仓库、前后端连通、多微服务协同或接口契约变更时，在 develop 阶段收尾或 verify 前置唤醒，负责依据各仓 setup.md 编排拉起本地环境、执行存活探活、真实调用核对并产出《本地集成联调与自测报告》（`integration-cases.md`，含人工验收指引），推进前由编排者向用户确认人工签章（无对应门禁 check 项）。
...
- Bash 冻结检查先用 `resolve()` 解析重定向、`cp` / `mv` / `install` 等静态写入目标，再检查冻结与（受守前缀下的）角色范围；`~` 在解析前展开；详见 [docs/permissions.md](docs/permissions.md#bash-分支绕过检查)。**根外写入不判角色**（2026-09-12）：`outside_targets` 只做记录，不据此拒绝。`git checkout`、外部编辑器和用户手改仍由门禁哈希校验兜底。
