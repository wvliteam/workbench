...
5. 摸到的**跨需求复用**的稳定事实（这个仓库怎么跑起来、怎么测、踩过什么坑）写进
   `repos/notes/<仓库>.md` —— 它跟 flow 无关，仓库不变它就不变，下个需求直接用；
   本次需求相关的现状（要动哪几处、撞什么风险）仍然只进 `current-state.md`。
   `status` 与 `role scopes` 会点名缺笔记的仓库。取不到证写「待补充」，不要猜。
   多仓库工作区里每个仓库一份，文件名用 `repos/` 下的目录名。

   这批活通常是独立任务：init 为每个还没有笔记的仓库建过 `仓库画像：<仓库>`
   （`role=analyst`、`phase=analyze`、`write-scopes=repos/notes/<仓库>.md`），
   编排者在初始化后单独派发。**它不属于任何需求** —— 需求驱动的 analyze 只看需求
