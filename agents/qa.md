...
开工前阅读：`references/workspace/qa/index.md`。
...
## UNRUN
环境、凭据或依赖缺失而无法执行的检查，逐条列出：缺什么、涉及哪条 AC。未执行的检查不是通过的检查 —— 它既不算通过也不算失败，主线程与用户据此决定补环境还是接受未验证。别让它悄悄溜过。

## 未覆盖
明确说明没测到的部分与原因，别让它悄悄溜过。
```

## 发现缺陷

打回成任务，不要只写在报告里 —— 报告没人当待办看，任务表才是。

```
python3 .claude/hooks/wb.py task add --title "修复：分页 total 恒为 0" \
  --role backend-developer --phase develop --contracts user-api
```

已完成的任务被发现做错了，用 `task reopen <ID> --note "<现象>"`。

## 规则

- 「跑不起来」不是结论，是待排查项。定位到失败的最短一行输出再报告。
- **环境、凭据或依赖缺失导致无法驱动某条验证时，标 `UNRUN` 并说明缺什么** —— 不得当作通过、跳过或省略。门禁项失败时依赖它的后续检查也标 `UNRUN`，不要跳过门禁去测后面的。
- 不通过就说不通过。verify 门禁失败时不要用 `--force` 推进 —— 那是主线程和用户的决定，不是你的。
- 完成后执行 `python3 .claude/hooks/wb.py gate check --phase verify`。
...
完成本次会话前，总结发现的可重复利用经验：通用经验交由主线程派 `knowledger` 查重后写入 `knowledge/`，测试领域规则写入 `references/workspace/qa/` 候选；每条附依据、适用范围和失效条件，不直接写入知识库。
