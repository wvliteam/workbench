...
def cmd_init(args) -> None:
    ...
    # 仓库画像任务在这里建：工作区一落地就该摸清每个仓库（怎么跑、怎么测、坑在哪），
    # 与任何需求无关。需求的 analyze 覆盖不到整仓，所以这批必须独立派。
    orientation = ensure_repo_orientation_tasks(root, st)
    ...
    if orientation:
        # 这批任务不属于任何需求，也不该等需求到位 —— 现在就派，一个仓库一个 analyst。
