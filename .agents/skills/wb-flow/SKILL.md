...
返回依赖已满足的一批任务（受 `max_parallel` 限制，默认 3）。**把这一批放在同一条消息里用多个 subagent 调用同时派出去** —— 前后端对着锁定的契约并行开发是这套流程的核心收益，串行派发等于白做前面的契约工作。
...
每批回来后，把 subagent 报的校验命令**自己跑一遍**，把命令与输出记进 `.workbench/artifacts/develop/verification.md`。用 `apply_patch`：先读出文件现有内容，再连着新的一段一起写回 —— 两个开发角色共用这一份，让 subagent 各自写会互相覆盖，而 shell 追加（`>> .workbench/...`）被守卫拦。
...
把 reviewer 报上来的「需要沉淀的规则」写进项目 AGENTS.md 或门禁规则，否则复盘白做。

## 汇报给用户

每轮只说：当前阶段、这轮做了什么、门禁结果、下一步、需要用户决定的事。不要复述 subagent 的完整报告 —— 用户看不到 subagent 输出，你转述关键结论就够，别转述过程。

需要用户决策时直接向用户提问：pm 报上来的阻塞待确认项、architect 的方案取舍、是否强推门禁、QA 报的缺陷是修还是接受。
