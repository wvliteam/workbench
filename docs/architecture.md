...
**猜不出名字的仓库没有默认开发角色认领。** 只要有一个仓库被认领，`repos/<仓库>/**` 这条分支就把默认范围钉在被认领的仓库上，于是 `shared` / `payments-core` 这类名字落在所有角色的默认范围之外。**但要分清执法边界**：产品源码（`repos/.source/**`）不在受守前缀内，守卫对它**不按角色执法**（`_check_write_target` 的 `if not guarded: return`），所以未认领仓库**不会在 develop 触发权限拒绝**——它是「没有推荐的开发角色可派」的**任务分工缺口**，不是写入硬拦。`unclaimed_repos()` 判定它、`init` 与 `role scopes` 当场点名，提醒在派 develop 任务时显式指定角色或补认领。判定按守卫自己的方式做：拿 `repos/.source/<项目>/<仓库>/src/probe.{ts,py}` 去撞两个开发角色的模式，撞不上就算没人认领。只看开发角色是因为 `qa` 的 `repos/*/tests/**` 覆盖所有仓库，而「只有 qa 能写它的测试目录」不构成认领。
