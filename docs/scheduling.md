...
`next --all` 最多返回 `max_parallel` 条。派发前先看这一批任务的**写入范围是否重叠** —— 同一批里两个 agent 改同一个文件会互相覆盖。

拆任务时用 `task add --write-scopes "目录/**,文件"` 声明写入范围，`next --all` 会据此跳过同批中范围有祖先/子路径关系的任务（继续挑选后面的不冲突任务），并在 JSON 里返回 `deferred_write_scope_conflicts` 说明谁被谁挤掉了。**没声明范围的历史任务不参与冲突判定**（保持旧行为）—— 范围判断只认显式相对路径前缀，不推断 agent 实际会改哪些文件；无法提前拆边界的任务不要并行。
...
无就绪任务时的文本输出会说明是哪种情况：`无就绪任务。 进行中：T3. 阻塞：T4. 失效：T5 — 上游被推翻，需 reopen 后重跑.` 或 `无就绪任务。 该阶段可以跑门禁了。`（`--json` 输出带 `tasks` / `doing` / `blocked` / `stale` 四键）。
...
`task start` 给任务挂一条 `lease_until`（`task_lease` 秒数，默认 3600），进入终态（`done` / `blocked` / `skipped` / `reopen`）时清除 —— 离开 `doing` 就不该有活动租约，留着过期值会让 status 对已 `done` 的任务虚报「租约过期」。租约过期**只在 `status` 与 `next` 提示，不自动抢占**：中断的 agent 是否还在跑、要不要重派，是编排者的判断，不是状态内核能替它做的决定 —— 与「解冻窗口在无 doing 任务时才收」同一条理由。
