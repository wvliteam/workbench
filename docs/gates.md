...
| 仓库画像已写 | `repos_notes_exist` | 无 `repos/` 布局时跳过；否则每个仓库都有画像三件套 `repos/<项目>/<仓库>/{overview,setup,test}.md` 且都不是空文件 | 画像与需求分析是两件事：需求驱动的 analyze 只覆盖相关部分，产不出整仓事实 —— 漏派、新增仓库、画像被删都在这里点名。**不逐条查「待补充」**：画像是长文，局部未取证是合规写法，逐条点名只会把警告刷成噪音 |
